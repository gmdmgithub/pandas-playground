...
def prepare_json_for_accounts():
    ...
    # TODO?? Mapping from JOINT.HOLDER?? or jonin
    ...
    #TODO!! incoming outcoming internal, external x4

    df['overdraft_flag'] = df['LINK.TO.LIMIT'].map(lambda x: x == 'YES')

    df['RE_CO'] = df['SECTOR.CU.NATCLI'].map(
        lambda x: 'RE' if x == 'RE' else 'CO')
    df['product_type_id'] = df.apply(lambda x: dm.product_type_id(x['owner_id'],
        x['product_type_id']+'|'+x['currency']+'|'+x['RE_CO']), axis=1)
    
    #Decission Tomasz Motyl - default value 03.10.2019
    df['swift_bic_code'] = 'BMBPBEBXXXX' # df['MNEMONIC']

    # TODO!!! field name is the name of the owner in the file
    df['name'] = 'MAIN ACCOUNT'

    # TODO!! information add migrated - Karolina Bulenda
